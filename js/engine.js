...
var Engine = function(global) {
    ...
    var doc = global.document,
        win = global.window,
        canvas = doc.getElementById('myCanvas'),//doc.createElement('canvas'),
    ...
    function main() {
        ...
        //if(){}点击了play再render
        begin(start);
        /*var stop = win.requestAnimationFrame(main);*/
     ...
     */
    function updateEntities(dt,stop) {
     ...
     * 函数调用一次。
     */
    function success(stop) {
        // 空操作
        if(Number(player.y) === 55){
            win.cancelAnimationFrame(stop);
            ctx.drawImage(Resources.get('images/Heart.png'),202,83*3+55);
    ...
    function fail(stop) {
        ...
        win.cancelAnimationFrame(stop);
        if(confirm("Game! Do you want to replay?")){
            player.x = 202;
            player.y = 83 * 4 + 55;
        }
    }
